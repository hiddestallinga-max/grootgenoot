...
-- Soft delete (prullenbak in de regiekamer): gezet = in de prullenbak,
-- null = actief. Definitief verwijderen doet een echte delete.
alter table public.aanmeldingen add column if not exists verwijderd_op timestamptz;

-- Eenmalige tekstmigratie: "bankieren" is uit de categorie gehaald
-- (risico op financieel misbruik); veilig om opnieuw te draaien.
update public.aanmeldingen
  set categorieen = array_replace(
    categorieen,
    'Hand- en spandiensten (was, bankieren, IT)',
    'Hand- en spandiensten (was, administratie, IT)'
  )
  where 'Hand- en spandiensten (was, bankieren, IT)' = any(categorieen);
...
-- Soft delete (prullenbak) voor koppelingen.
alter table public.koppelingen add column if not exists verwijderd_op timestamptz;

-- Restant van het oude percentagemodel; wordt nergens meer gebruikt.
alter table public.koppelingen drop column if exists service_pct;
...
-- Vooraankondiging (SEPA-prenotificatie): de factuur wordt eerst gemaild met
-- een concrete afschrijfdatum; de cron voert de incasso uit zodra die datum
-- is bereikt. Nieuwe statussen: 'aangekondigd' en 'geannuleerd'.
alter table public.facturen add column if not exists incasso_vanaf date;
...
alter table public.facturen add constraint facturen_status_check
  check (status in ('aangemaakt','aangekondigd','in_behandeling','betaald','mislukt','geannuleerd'));

-- Facturen mogen NOOIT verdwijnen (fiscale bewaarplicht van 7 jaar), ook niet
-- als de koppeling of persoon definitief wordt verwijderd. Daarom: koppeling-
-- verwijzing nullable en "on delete set null" in plaats van cascade.
alter table public.facturen alter column koppeling_id drop not null;
alter table public.facturen drop constraint if exists facturen_koppeling_id_fkey;
alter table public.facturen add constraint facturen_koppeling_id_fkey
  foreign key (koppeling_id) references public.koppelingen(id) on delete set null;

-- Welke uren op welke factuur staan. Zo kan een mislukte incasso precies de
-- juiste uren terugzetten (en nooit uren van eerdere, betaalde facturen).
alter table public.uren add column if not exists factuur_id uuid;
alter table public.uren drop constraint if exists uren_factuur_id_fkey;
alter table public.uren add constraint uren_factuur_id_fkey
  foreign key (factuur_id) references public.facturen(id) on delete set null;
create index if not exists uren_factuur_idx on public.uren (factuur_id);
